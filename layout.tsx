...
/* ═══════════════ FIFA FUT CARD ═══════════════ */
function FutCard({ data, size='md', glow=false, mini=false, scale=1 }:
  { data:SlotData; size?:'sm'|'md'|'lg'; glow?:boolean; mini?:boolean; scale?:number }) {
  const c = POS_COLORS[data.pos];
  const baseDim = size==='lg' ? 140 : size==='sm' ? 95 : 118;
  const dim = Math.round(baseDim * scale);
  const fs  = Math.round((size==='lg' ? 15 : size==='sm' ? 12  : 14) * scale);
  const nfs = Math.round((size==='lg' ? 17 : size==='sm' ? 13  : 15) * scale);
  const nmaxw = Math.round((size==='lg' ? 124 : size==='sm' ? 80 : 102) * scale);
...
{ slotRole:string; slotLabel:string; data?:SlotData; subData?:SlotData;
  ...
  const nameFs = Math.round(13 * cardScale);
  const subFs  = Math.round(13 * cardScale);
  const addSubFs = Math.round(12 * cardScale);
  ...
  return (
    ...
    >
      {/* STARTER CARD */}
      {data ? (
        <div draggable onDragStart={e=>{ onDragStartCard(e,slotRole); (e.currentTarget as HTMLElement).style.opacity='0.35'; (e.currentTarget as HTMLElement).style.transform='scale(1.12) rotate(-2deg)'; }} onDragEnd={e=>{ (e.currentTarget as HTMLElement).style.opacity='1'; (e.currentTarget as HTMLElement).style.transform=''; }}
          onClick={onClick}
          style={{ cursor:'grab', filter: isDragTarget ? 'brightness(1.25)' : 'none',
            outline: isDragTarget ? '3px solid #00ff87' : 'none', outlineOffset:4, borderRadius:8,
            transition:'transform 0.15s cubic-bezier(0.34,1.56,0.64,1), opacity 0.15s, filter 0.15s',
            transform: isDragTarget ? 'scale(1.05)' : 'scale(1)' }}>
          <FutCard data={data} size="md" glow={hovered||isDragTarget} scale={cardScale}/>
        </div>
      ) : (
        <div onClick={onClick}
          onDragOver={e=>e.preventDefault()} onDrop={e=>onDropOnCard(e,slotRole)}
          style={{ width:Math.round(118*cardScale), height:Math.round(163*cardScale), borderRadius:8, background: isDragTarget?'rgba(0,255,135,0.12)':'rgba(255,255,255,0.05)',
          ...
          <span style={{fontSize:14,color:'rgba(255,255,255,0.3)',fontWeight:700}}>{slotLabel}</span>
        </div>
      )}
      ...
      {data && (
        <div style={{ display:'flex', alignItems:'center', gap:4, background:'rgba(0,0,0,0.85)', backdropFilter:'blur(8px)',
          border:`1px solid ${POS_COLORS[data.pos].accent}66`, borderRadius:5, padding:'3px 9px', maxWidth:140 }}>
          ...
          <button onClick={e=>{e.stopPropagation();onRemove();}} style={{background:'none',border:'none',color:'#ff444477',cursor:'pointer',fontSize:13,padding:0,lineHeight:1,flexShrink:0}}
            onMouseEnter={e=>(e.currentTarget.style.color='#ff4444')} onMouseLeave={e=>(e.currentTarget.style.color='#ff444477')}>×</button>
        </div>
      )}

      {/* SUB SLOT — name only chip */}
      {data && (
        subData ? (
          <div
            draggable onDragStart={onDragStartSub} onClick={onEditSub}
            style={{
              display:'flex', alignItems:'center', gap:6,
              background:'rgba(0,0,0,0.80)', backdropFilter:'blur(8px)',
              border:`1.5px solid ${POS_COLORS[subData.pos].accent}77`,
              borderRadius:20, padding:'5px 12px 5px 8px', marginTop:4,
              cursor:'grab', maxWidth:150,
            }}>
            <div style={{
              width:18, height:18, borderRadius:'50%', flexShrink:0,
              background:POS_COLORS[subData.pos].bg,
              display:'flex', alignItems:'center', justifyContent:'center',
            }}>
              <span style={{fontSize:9,fontWeight:900,color:POS_COLORS[subData.pos].accent,lineHeight:1}}>{subData.pos[0]}</span>
            </div>
            <span style={{
              fontSize:subFs, fontWeight:700, color:'#f0f6fc',
              whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis',
            }}>{subData.name}</span>
            <button onClick={e=>{e.stopPropagation();onRemoveSub();}}
              style={{background:'none',border:'none',color:'#ff444077',cursor:'pointer',fontSize:14,padding:0,lineHeight:1,flexShrink:0,marginLeft:2}}
              onMouseEnter={e=>(e.currentTarget.style.color='#ff4444')}
              onMouseLeave={e=>(e.currentTarget.style.color='#ff444077')}>×</button>
          </div>
        ) : (
          <button onClick={onAddSub}
            style={{
              background:'rgba(0,0,0,0.45)', border:'1px dashed rgba(255,255,255,0.15)',
              borderRadius:20, padding:'5px 14px', fontSize:addSubFs, color:'rgba(255,255,255,0.3)',
              cursor:'pointer', display:'flex', alignItems:'center', gap:4, marginTop:4,
              transition:'all 0.15s',
            }}
            onMouseEnter={e=>{e.currentTarget.style.borderColor='#00ff8888';e.currentTarget.style.color='#00ff87';}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.15)';e.currentTarget.style.color='rgba(255,255,255,0.3)';}}>
            <span style={{fontSize:15,lineHeight:1}}>+</span> Yedek
          </button>
        )
      )}
    </div>
...
export default function LineupBuilder() {
  ...
  const [cardScale,setCardScale]         = useState(1.0);   // yazı/kart boyutu çarpanı
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const canvas = await h2c(pitchRef.current, {
        scale: 2,
        useCORS: true,
        allowTaint: true,
      ...
      final.height = canvas.height + bannerH;
      const ctx = final.getContext('2d')!;
      // Banner bg
      ctx.fillStyle = '#0d1117';
      ctx.fillRect(0, 0, final.width, bannerH);
      // Team name
      ctx.fillStyle = '#f0f6fc';
      ctx.font = `bold ${bannerH * 0.38}px Georgia, serif`;
      ctx.textAlign = 'left';
      ctx.fillText(teamName.toUpperCase() || 'TAKIMIM', 24, bannerH * 0.62);
      // Formation badge
      ctx.fillStyle = '#00ff87';
      ctx.font = `bold ${bannerH * 0.28}px Inter, sans-serif`;
      ctx.textAlign = 'right';
      ctx.fillText(formation, final.width - 24, bannerH * 0.62);
      // Thin green line under banner
      ctx.fillStyle = '#00ff87';
      ctx.fillRect(0, bannerH - 2, final.width, 2);
      // Pitch
      ctx.drawImage(canvas, 0, bannerH);
      // Footer
      ctx.fillStyle = 'rgba(0,0,0,0.6)';
      ctx.fillRect(0, final.height - 28, final.width, 28);
      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.font = `${bannerH * 0.18}px Inter, sans-serif`;
      ctx.textAlign = 'center';
      ctx.fillText('Lineup Builder · ' + filledCount + ' oyuncu · ' + subsCount + ' yedek', final.width / 2, final.height - 10);
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      const bh = 72;
      final.width = canvas.width;
      final.height = canvas.height + bh;
      const ctx = final.getContext('2d')!;
      // Header
      ctx.fillStyle = '#0d1117';
      ctx.fillRect(0, 0, final.width, bh);
      ctx.fillStyle = '#00ff87';
      ctx.fillRect(0, bh - 3, final.width, 3);
      ctx.fillStyle = '#f0f6fc';
      ctx.font = `bold ${bh * 0.42}px Georgia, serif`;
      ctx.textAlign = 'left';
      ctx.fillText((teamName || 'TAKIMIM').toUpperCase(), 28, bh * 0.64);
      ctx.fillStyle = '#00ff87';
      ctx.font = `bold ${bh * 0.3}px Inter, sans-serif`;
      ctx.textAlign = 'right';
      ctx.fillText(formation, final.width - 28, bh * 0.64);
      // Pitch
      ctx.drawImage(canvas, 0, bh);
      // Footer
      ctx.fillStyle = 'rgba(0,0,0,0.65)';
      ctx.fillRect(0, final.height - 32, final.width, 32);
      ctx.fillStyle = 'rgba(255,255,255,0.3)';
      ctx.font = `${bh * 0.2}px Inter, sans-serif`;
      ctx.textAlign = 'center';
      ctx.fillText(`${filledCount} oyuncu · ${subsCount} yedek · Lineup Builder`, final.width / 2, final.height - 12);
      setExportUrl(final.toDataURL('image/png'));
            ...
            {curF.positions.map(slot=>{
              ...
              return (
                <div key={slot.role} style={{marginBottom:4}}>
                  ...
                  {/* Sub row — plain name chip */}
                  {p && (
                    <div style={{display:'flex',alignItems:'center',gap:5,padding:'3px 8px 3px 32px'}}>
                      <div style={{width:1,height:18,background:'#21262d',flexShrink:0,marginRight:2}}/>
                      {sub ? (
                        <div onClick={()=>setEditSubSlot(slot.role)}
                          style={{display:'flex',alignItems:'center',gap:5,flex:1,cursor:'pointer',padding:'3px 7px',borderRadius:5,
                            background:'#0f1318',border:`1px solid ${POS_COLORS[sub.pos].accent}33`}}
                          onMouseEnter={e=>(e.currentTarget.style.background='#161b22')}
                          onMouseLeave={e=>(e.currentTarget.style.background='#0f1318')}>
                          <span style={{fontSize:8,fontWeight:800,color:POS_COLORS[sub.pos].accent,background:`${POS_COLORS[sub.pos].bg}cc`,borderRadius:3,padding:'1px 4px',flexShrink:0}}>{sub.pos}</span>
                          {sub.number && <span style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontWeight:700,flexShrink:0}}>#{sub.number}</span>}
                          <span style={{fontSize:11,color:'#c9d1d9',fontWeight:600,flex:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{sub.name}</span>
                          <button onClick={e=>{e.stopPropagation();setSubs(prev=>{const n={...prev};delete n[slot.role];return n;});}}
                            style={{background:'none',border:'none',color:'#ff444477',cursor:'pointer',fontSize:12,padding:0,flexShrink:0}}
                            onMouseEnter={e=>(e.currentTarget.style.color='#ff4444')}
                            onMouseLeave={e=>(e.currentTarget.style.color='#ff444477')}>×</button>
                        </div>
                      ) : (
                        <button onClick={()=>setAddSubSlot(slot.role)}
                          style={{fontSize:10,color:'#8b949e',background:'none',border:'1px dashed #21262d',borderRadius:4,padding:'3px 9px',cursor:'pointer',transition:'all 0.12s'}}
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:'#0a0e14'}}>
        ...
        <div style={{padding:'9px 18px',borderBottom:'1px solid #21262d',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117',flexShrink:0}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <button onClick={()=>setShowSettings(s=>!s)} style={{background:showSettings?'#00ff8722':'#161b22',border:`1px solid ${showSettings?'#00ff8755':'#21262d'}`,color:showSettings?'#00ff87':'#f0f6fc',padding:'6px 12px',borderRadius:6,fontSize:11,fontWeight:600,cursor:'pointer'}}>⚙️ Ayarlar</button>
            <button onClick={handleScreenshot} style={{background:'linear-gradient(135deg,#6366f1,#8b5cf6)',border:'none',color:'#fff',padding:'6px 14px',borderRadius:6,fontSize:11,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:5}}>📸 Ekran Görüntüsü</button>
        ...
        {/* Settings Panel */}
        {showSettings && (
          <div style={{background:'#0d1117',borderTop:'1px solid #21262d',padding:'10px 18px',display:'flex',alignItems:'center',gap:20,flexShrink:0,flexWrap:'wrap'}}>
            <div style={{fontSize:11,fontWeight:700,color:'#8b949e',letterSpacing:1}}>⚙️ AYARLAR</div>
            {/* Card scale */}
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <span style={{fontSize:11,color:'#8b949e',fontWeight:600,whiteSpace:'nowrap'}}>🃏 Kart Boyutu</span>
              ...
              <div style={{width:44,textAlign:'center',fontSize:12,fontWeight:700,color:'#00ff87',background:'#161b22',borderRadius:5,padding:'3px 0',border:'1px solid #21262d'}}>{Math.round(cardScale*100)}%</div>
              ...
              <div style={{display:'flex',gap:4}}>
                {[0.7,0.85,1.0,1.2,1.4].map(v=>(
                  <button key={v} onClick={()=>setCardScale(v)} style={{padding:'2px 7px',background:cardScale===v?'#00ff87':'#161b22',border:'none',borderRadius:4,color:cardScale===v?'#000':'#8b949e',fontSize:10,fontWeight:700,cursor:'pointer'}}>{Math.round(v*100)}%</button>
                ))}
              </div>
            </div>
          </div>
        ...
        {/* Export Modal */}
        {showExport && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:400,backdropFilter:'blur(8px)'}}
            onClick={e=>{if(e.target===e.currentTarget){setShowExport(false);setExportUrl(null);}}}>
            <div style={{background:'#0d1117',border:'1px solid #30363d',borderRadius:16,padding:24,maxWidth:520,width:'90vw',boxShadow:'0 24px 64px rgba(0,0,0,0.8)'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              ...
              {exporting ? (
                <div style={{textAlign:'center',padding:'40px 0'}}>
                  ...
                  <div style={{fontSize:14,color:'#8b949e'}}>Görüntü oluşturuluyor...</div>
                </div>
              ) : exportUrl ? (
                <>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    <a href={exportUrl} download={`${teamName||'lineup'}-${formation}.png`}
                      style={{flex:1,background:'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff',padding:'11px',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer',textDecoration:'none',textAlign:'center',display:'block'}}>
                      ⬇️ PNG İndir
                    </a>
                    <button onClick={()=>{ if(exportUrl){ const w=window.open(); if(w){w.document.write(`<img src="${exportUrl}" style="max-width:100%">`);} }}} style={{flex:1,background:'#21262d',border:'1px solid #30363d',color:'#f0f6fc',padding:'11px',borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer'}}>
                      🔗 Yeni Sekmede Aç
                    </button>
                  </div>
                </>
